...
public class SC_EnemySpawner : MonoBehaviour
{
    ...
    public float spawnInterval = 200; //Spawn new enemy each n seconds
            ...
            if(Time.time > nextSpawnTime)
            {
                ...
                if(totalEnemiesSpawned < enemiesToEliminate)
                {
                    for (int i = 0; i < spawnPoints.Length; i++) {
                        Transform randomPoint = spawnPoints[i]; 
    ...
    void OnGUI()
    {
        ...
        // GUI.Box(new Rect(Screen.width - 165, Screen.height - 35, 150, 25), " Инсендио");
